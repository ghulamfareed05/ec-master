...
const NavbarComponent: React.FC<Subcategory> = ({ subcategoryName, className, types }) => {
  ...
  return (
    <div className="relative group" onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
      <NavLink href={{ pathname: '/meat', query: { subcategory: subcategoryName } }} className={`text-white font-medium ${className} hover:text-red-500 group-hover:border-b-red-700 group-hover:border-b-4 pb-[18px]  transition-all ease-in-out duration-75`}>
        {subcategoryName}
      </NavLink>
      {isHovered && types && (
        <div className="absolute left-0 mt-6 w-48 bg-white shadow-lg">
          ...
          {types.map((type, id) => (
            <div  key={id} onMouseEnter={() => handleSubmenuEnter(id)} onMouseLeave={handleSubmenuLeave}>
              <div className="px-5 py-4 text-xs font-bold border-b-2 hover:bg-red-100 hover:border-b-0 hover:text-red-600  hover:border-l-4 hover:border-red-600 flex justify-between items-center cursor-pointer"><NavLink href={{pathname:'/meat',query:{type:type.typeName}}}>{type.typeName}</NavLink>{type.variants?<IoMdArrowDropright className='text-sm'/>:null}</div>
              {hoveredSubmenu === id && type.variants && (
                <div className="absolute left-48 top-0 mt-0 w-max bg-white shadow-lg">
                  {type.variants.map((variant, id) => (
                    <div key={id}>
                      <NavLink href={{ pathname:'/meat', query: { variant: variant.variantName } }} className="block px-5 py-4 hover:bg-red-100 border-b-2 hover:border-l-4 hover:border-red-600 hover:border-b-0 text-xs font-bold">
                        {variant.variantName}
                      </NavLink>
                    </div>
                  ))}
