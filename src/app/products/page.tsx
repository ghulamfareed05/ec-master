...
const Product: React.FC = () => {
  ...
  const [decodecategory, setdecodecategory] = useState("");
      ...
      try {
        if (category) {
          ...
          const response = await axios.get(
            `http://localhost:3000/product/getProductsByCategory?category=${category}`
          );
          setProducts(response.data);
          setdecodecategory(decodeURIComponent(category));
        } else if (subcategory) {
          const response = await axios.get(
            `http://localhost:3000/product/getProductsBySubcategory?subcategory=${subcategory}`
          );
          setProducts(response.data);
          setdecodecategory(decodeURIComponent(subcategory));
        } else if (type) {
          const response = await axios.get(
            `http://localhost:3000/product/getProductsByType?type=${type}`
          );
          setProducts(response.data);
          setdecodecategory(decodeURIComponent(type));
        } else {
  ...
  }, [category, subcategory,type]); // Include category in the dependency array

  return (
    <div className="mt-36">
      <Divider content={decodecategory} />
